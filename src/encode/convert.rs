...
use windows::Win32::Graphics::Direct3D11::{
    ...
    D3D11_SRV_DIMENSION_TEXTURE2D, D3D11_TEX2D_SRV, D3D11_TEX2D_UAV, D3D11_TEXTURE2D_DESC,
    D3D11_UAV_DIMENSION_TEXTURE2D, D3D11_UNORDERED_ACCESS_VIEW_DESC, D3D11_USAGE_DEFAULT,
...
impl BgraToNv12Converter {
    pub fn new(device: &ID3D11Device, width: u32, height: u32) -> anyhow::Result<Self> {
        unsafe {
            ...
            let y_texture = device.CreateTexture2D(
            ...
            let uv_texture = device.CreateTexture2D(
                &D3D11_TEXTURE2D_DESC {
                    Width: width / 2,
                    Height: height / 2,
                    MipLevels: 1,
            ...
            let nv12_texture = device.CreateTexture2D(
                &D3D11_TEXTURE2D_DESC {
                    Width: width,
                    Height: height,
                    MipLevels: 1,
    ...
    pub fn convert(&self, bgra_texture: &ID3D11Texture2D) -> anyhow::Result<&ID3D11Texture2D> {
        unsafe {
            ...
            let srv = self
                .device
                .CreateShaderResourceView(bgra_texture, Some(&srv_desc))?;
